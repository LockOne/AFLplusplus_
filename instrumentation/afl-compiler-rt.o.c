...
static u8 __afl_callee_initial[CALLEE_MAP_SIZE];
u8       *__afl_callee_map_ptr = __afl_callee_initial;
...
static void __afl_map_shm(void) {
  ...
  id_str = getenv(CALLEE_SHM_ENV_VAR);

  if (__afl_debug) {
    fprintf(stderr, "DEBUG: callee id_str %s\n",
...
static void __afl_unmap_shm(void) {
  ...
  if (id_str) { shmdt((void *)__afl_callee_map_ptr); }
...
void __sanitizer_cov_pcs_init(const uintptr_t *pcs_beg,
                              const uintptr_t *pcs_end) {
  // If for whatever reason, we cannot get dlinfo here, then pc_guard_init
  // also couldn't get it and we'd end up attributing to the wrong module.
  Dl_info dlinfo;
    ...
    while (start < end) {
      ...
      if (pc_filter && !mod_info->next) {
        ...
        if (strstr(PcDescr, pc_filter)) {
          if (__afl_debug)
            fprintf(stderr,
                    "DEBUG: Selective instrumentation match: %s (PC %p Index "
                    "%u)\n",
...
void __sanitizer_cov_trace_pc_guard_init(uint32_t *start, uint32_t *stop) {
  ...
  if (__afl_debug) {
    fprintf(stderr,
            "DEBUG: Running __sanitizer_cov_trace_pc_guard_init: %p-%p (%lu "
            "edges) "
            "after_fs=%u *start=%u\n",
...
void __cmplog_ins_hook1(uint8_t arg1, uint8_t arg2, uint8_t attr) {
  ...
  k = (uintptr_t)(default_hash((u8 *)&k, sizeof(uintptr_t)) & (CMP_MAP_W -
  1));
