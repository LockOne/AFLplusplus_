...
void afl_shm_deinit(sharedmem_t *shm) {
  ...
  shmctl(shm->callee_shm_id, IPC_RMID, NULL);
...
u8 *afl_shm_init(sharedmem_t *shm, size_t map_size,
  ...
  shm->callee_shm_id = shmget(IPC_PRIVATE, CALLEE_MAP_SIZE,
                              IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);

  if (shm->callee_shm_id < 0) {
  ...
  shm_str = alloc_printf("%d", shm->callee_shm_id);
  setenv(CALLEE_SHM_ENV_VAR, shm_str, 1);
  ck_free(shm_str);
